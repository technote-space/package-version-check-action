...
const updatePackageVersion = async (context) => {
    ...
    logger.displayStdout(misc_1.getReplaceResultMessages(await replace_in_file_1.replaceInFile({
        files: path,
...
const getUpdateBranch = async (logger, context) => {
    ...
    if (tagName) {
        ...
        if (!(await command_1.getBranchesByTag(tagName)).includes(branch)) {
...
const commit = async (octokit, context) => {
    ...
    const branch = await exports.getUpdateBranch(logger, context);
    if (false === branch) {
        return false;
    }
    ...
    await helper.commit(misc_1.getPackageDir(), misc_1.getCommitMessage(), [misc_1.getPackageFileName()]);
